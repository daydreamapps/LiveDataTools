...
// This  class was taken from https://github.com/jraska/livedata-testing

class TestObserver<T: Any?> : Observer<T?> {
    ...
    fun assertValueHistory(vararg values: T): TestObserver<T> {
        ...
        if (size != values.size) {
            throw fail(
                "Value count differs; expected: " + values.size + " " + Arrays.toString(
                    values
                )
